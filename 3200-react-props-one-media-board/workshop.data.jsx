...
{/*. 1 */}
        We will be defining a component to display a video from
                the Vimeo website, so we first import React from the react library otherwise we can't
                use jsx.

{/*. 2 */}
        A component is just a function that returns something which is displayable.
        The function will be called by React and the argument passed will be an object
        whose properties are the props. These props are determined when the component is used.
         We are assuming that a videoId prop is passed. Looking at App.jsx, we see that the prop is indeed passed.

{/*. 3 */}
        The functional component returns an iframe that contains the video. Notice that the src attribute
        ...
        of <code>{`videoId`}</code> is going to be undefined and the src attribute of the iframe
...
{/*. 1 */}
        You'll be using this component to play sound. It uses the audio tag, which
                    ...
                    It expects a url prop.

{/*. 2 */}
        Every time you use this component, you need
                           to give it the prop called url. This prop determines the sound location. If you don't
            give this component a url prop when you use it in your jsx, the value of <code>{`url`}</code> is going to be
...
{/*. 3 */}
        ...
        The name of the function will appear in the react devtools.
...
{/*. q */}
        In the functional component in App.jsx we use the array map method. Why do we use this method?
